...
class fsModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            print("--- [INFO] SETUP FREEZE FOR ENCODER ---")
            freeze_count = 0
            # 
            for name, param in self.netG.named_parameters():
                # Trong SimSwap, encoder thường là các lớp 'conv_first', 'down'
                if 'down' in name or 'conv_first' in name or 'feature' in name:
                    param.requires_grad = False  # ĐÓNG BĂNG
                    freeze_count += 1
                    print(f"Frozen layer: {name}") # Debug: In ra tên layer đã đóng băng
                else:
                    param.requires_grad = True   # MỞ (cho phép train Decoder)
            
            print(f"--- ĐÃ ĐÓNG BĂNG: {freeze_count} layers (Giữ nguyên kiến thức cũ) ---")
            print("--- CÁC LAYER CÒN LẠI SẼ ĐƯỢC TRAIN (Học mặt của bạn) ---")
